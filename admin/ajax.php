        ...
        case 'student':
         ...
         //   $enrs = substr($s,8);
            ...
            foreach ($result as $key => $value) {
                ...
                $class = isset($usermeta['class']) ? get_post(array('id' => $usermeta['class'])) : null;
                $section = isset($usermeta['section']) ? get_post(array('id' => intval($usermeta['section']))) : null;

                $class_title = $class ? $class->title : 'Unknown Class';
                $section_title = $section ? $section->title : 'Unknown Section';
                $class_info = $class_title . ' (' . $section_title . ')';
                ...
                $data['data'][] = [
                    ...
                    'photo' => $img,
                    'name' => $value['name'],
                    'dob' => isset($usermeta['dob']) ? $usermeta['dob'] : 'N/A',
                    'father_name' => isset($usermeta['father_name']) ? $usermeta['father_name'] : 'N/A',
                    'mother_name' => isset($usermeta['mother_name']) ? $usermeta['mother_name'] : 'N/A',
                    'doa' => isset($usermeta['doa']) ? $usermeta['doa'] : 'N/A',
                    'address' => isset($usermeta['address']) ? $usermeta['address'] : 'N/A',
                    'action' => 
                    '<a href="user-account.php?user=student&action=view&id=' . $value['id'] . '" class="btn btn-sm btn-success"><i class="fa fa-eye"></i></a>
                    <a href="user-account.php?user=student&action=edit&id=' . $value['id'] . '" class="btn btn-sm btn-info"><i class="fa fa-pencil-alt"></i></a>
                    <a href="user-account.php?user=student&action=trash&id=' . $value['id'] . '" class="btn btn-sm btn-danger trash-user"><i class="fa fa-trash"></i></a>',
        ...
        case 'parent':
            ...
            foreach ($result as $key => $value) {
               ...
               // $children = unserialize($usermeta['children']);
                $children = isset($usermeta['children']) ? unserialize($usermeta['children']) : [];
                $children_data = '';
